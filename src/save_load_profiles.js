...
document.addEventListener('click', function (e) {
  ...
  if (!atkDropdown.contains(e.target) && !atkDropdownToggle.contains(e.target)) {
    closeDropdown('atk-profile-dropdown-list'); // Close attacker dropdown if clicked outside
  }

  if (!defDropdown.contains(e.target) && !defDropdownToggle.contains(e.target)) {
...
function save_def_profile() {
  ...
  let defprofile_map = JSON.parse(localStorage.getItem('def_profile_map')) || {};
...
function update_def_profile_dropdown() {
  ...
  let toggleButton = document.querySelector('#def-profile-dropdown .custom-dropdown-selected');
...
function highlightSelectedDefProfile(selectedProfile) {
  let profileItems = document.querySelectorAll('#def-profile-dropdown-list div');
  profileItems.forEach(function (item) {
    item.classList.remove('selected');
    if (selectedProfile && item.querySelector('span').textContent === selectedProfile) {
      item.classList.add('selected');
