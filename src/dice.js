    ...
    for (var n = 0; n <= w; n++) {
      ...
      for (var m = 0; m < rolls.mortal[w].length; m++) {
        ...
        for (var d = 0; d < damage.length; d++) {
          ...
          results.mortal[w - n][m + d] +=
            rolls.mortal[w][m] * n_six_prob * damage[d];
        }
      }
    }
  }
...
function hits_of_6_add_hits(
  ...
  for (var h = 0; h < hits.normal.length; h++) {
    ...
    for (var n = 0; n <= h; n++) {
      ...
      for (
        var bonus_hits = 0;
        bonus_hits < bonus_hits_prob.length;
        bonus_hits++
      ) {
        ...
        for (var b = 0; b <= bonus_hits * n; b++) {
          ...
          var six_delta =
            hits.normal[h] * n_six_hit_prob * b_prob * bonus_val_chance;
...
function calc_wound_prob(
  ...
  if (hit_abilities['autowound']) {
    var hit_six_chance = hit_prob.six_chance / hit_prob.pass_chance;
    wound_prob.pass_chance =
      1.0 * hit_six_chance + wound_prob.pass_chance * (1.0 - hit_six_chance);
...
function do_saves(
  ...
  if (wound_abilities['pierce']) {
    ...
    save_prob.pass_chance =
      wound_six_chance * ap_save_prob.pass_chance +
      (1 - wound_six_chance) * save_prob.pass_chance;
    save_prob.fail_chance =
      wound_six_chance * ap_save_prob.fail_chance +
      (1 - wound_six_chance) * save_prob.fail_chance;
  }
    ...
    for (var d = 0; d < hit_damage.length; d++) {
      ...
      for (var m = 0; m < unsaved.mortal[n].length; m++) {
        for (var mortals = 0; mortals <= m; mortals++) {
          ...
          damage.normal[dam] +=
            hit_damage[d] * unsaved.mortal[n][m] * n_mortal_prob;
        }
      }
    }
  }
        ...
        for (var w = 0; w < hit_killed[k].length; w++) {
          ...
          for (var m = 0; m < unsaved.mortal[n].length; m++) {
            for (var mortals = 0; mortals <= m; mortals++) {
              ...
              killed.normal[kills] +=
                hit_killed[k][w] * unsaved.mortal[n][m] * n_mortal_prob;
            }
          }
        }
      }
...
function log_prob_array(label, prob) {
  ...
  if (prob.mortal && prob.normal.length != prob.mortal.length) {
    console.error(
      'Mismatched lengths: ' + prob.normal.length + ' != ' + prob.mortal.length,
    );
  }
  ...
  for (var w = 0; w < prob.normal.length; w++) {
    ...
    if (Math.abs(prob.normal[w] - mortal_row_sum) > 0.0001) {
      console.error(
        'Mortal probabilities do not sum to normal probability: ' +
          mortal_row_sum +
          ' != ' +
          prob.normal[w],
      );
...
function graph(raw_data, title, chart_name) {
  ...
  if (mortal.length <= 1) {
    ...
    for (var m = 0; m < mortal.length; m++) {
      ...
      cumulative_mortal_data.push({
        x: m + 0.5,
        y: Math.round(cumulative_mortal * 10) / 10.0,
      });
...
function generate_permalink_40k() {
  ...
  for (var i = 0; i < fields_40k.length; i++) {
    if (document.getElementById(fields_40k[i]).value) {
      pairs[pairs.length] =
        fields_40k[i] + '=' + document.getElementById(fields_40k[i]).value;
    }
  }
  ...
  for (var i = 0; i < selects_40k.length; i++) {
    if (document.getElementById(selects_40k[i]).value) {
      pairs[pairs.length] =
        selects_40k[i] + '=' + document.getElementById(selects_40k[i]).value;
    }
  }
          ...
          callbacks: {
            ...
            label: function (item) {
              if (item.datasetIndex == DATASET_EXPECTED) {
                ...
                return (
                  item.dataset.label.replace('{n}', Math.floor(item.parsed.x)) +
                  item.parsed.y +
                  '%'
                );
              }
